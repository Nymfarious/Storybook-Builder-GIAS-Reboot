...
import { Users, Image as ImageIcon, Box, Music, Tag as TagIcon, Check, X, Search } from 'lucide-react';
...
export const AssetLibrary: React.FC = () => {
  ...
  const saveTag = () => {
    ...
    setAssets(prev => prev.map(a => {
      if (a.id === selectedAssetId) {
        ...
        if (editingTagIndex !== null && editingTagIndex >= 0) {
          updatedTags[editingTagIndex] = tagData;
        } else {
  ...
  const startEditTag = (index: number, tag: Tag) => {
  ...
  const Tabs = () => (
    <div className="flex gap-1 mb-3 bg-black/20 p-1 rounded-lg">
      {[
        ...
        <button
          key={tab.id}
          onClick={() => { setActiveTab(tab.id as AssetCategory); setSelectedAssetId(null); setIsAddingTag(false); }}
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 min-h-[60px] scrollbar-thin scrollbar-thumb-gray-700">
        {filteredAssets.map(asset => (
          <button
            key={asset.id}
            onClick={() => { setSelectedAssetId(asset.id); setIsAddingTag(false); setEditingTagIndex(null); }}
      ...
      {selectedAsset && (
        <div 
          className="bg-black/20 rounded-lg p-3 border border-white/5 animate-in"
        ...
        >
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <button 
              onClick={() => { setIsAddingTag(!isAddingTag); setEditingTagIndex(null); setNewTagLabel(''); }}
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {selectedAsset.tags.map((tag, i) => (
              <div 
                ...
                className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-medium text-white shadow-sm cursor-pointer border ${editingTagIndex === i ? 'ring-1 ring-white border-white' : 'border-transparent hover:border-white/50'}`}
          ...
          {isAddingTag && (
            <div className="mb-3 p-2 bg-black/40 rounded border border-gray-700 space-y-2">
              <input 
                ...
                className="w-full bg-gray-800 text-white text-[10px] px-2 py-1 rounded border border-gray-600 outline-none focus:border-primary" 
                autoFocus
              />
              <div className="flex gap-1">
                <select 
                  ...
                  className="flex-1 bg-gray-800 text-white text-[10px] px-1 rounded border border-gray-600"
                >
                ...
                <button 
                  ...
                  className="px-2 bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white rounded"
                >
                  <Check size={12} />
                </button>
              </div>
          ...
          {/* Character Stack or Single Preview */}
          {selectedAsset.category === 'character' && selectedAsset.stack ? (
            ...
            <div 
              draggable 
              onDragStart={(e) => handleDragStart(e, (selectedAsset.previewUrl || selectedAsset.thumbnail), selectedAsset.category === 'audio' ? 'audio' : 'image')}
            ...
            >
              {selectedAsset.category === 'audio' ? (
                <Music size={32} className="text-gray-500 group-hover:text-white transition-colors" />
              ) : (
                <img src={selectedAsset.previewUrl || selectedAsset.thumbnail} alt="Preview" className="w-full h-full object-cover" />
              )}
